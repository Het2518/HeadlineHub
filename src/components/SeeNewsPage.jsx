...
import { ArrowLeft, Calendar, Share2, Sun, Moon } from 'lucide-react';
...
const SeeNewsPage = () => {
  ...
  <div className="container mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <Link to="/" className={`flex items-center ${darkMode ? 'text-gray-300' : 'text-gray-600'}`} aria-label="Go Back">
      ...
      <main className="container mx-auto px-4 py-8">
        <article className={`rounded-lg shadow-md overflow-hidden ${darkMode ? 'dark' : ''}`}>
          ...
          <div className={`p-6 ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'} `}>
            ...
            <p className={`text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-line ${darkMode ? 'bg-gray-800' : ''}`}>
              {newsContent}
            </p>
          </div>
        </article>
